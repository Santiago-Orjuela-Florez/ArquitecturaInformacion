    ...
    <div class="table-container">
        ...
        <div class="px-8 pb-8">
            <div class="relative flex justify-between items-end mb-6">
                <div class="w-24 h-24 -mt-12 rounded-full bg-white border-4 border-white flex items-center justify-center shadow-lg overflow-hidden">
                    <div class="w-full h-full bg-red-100 text-red-600 flex items-center justify-center text-3xl font-bold">
                        {{ substr($user->name ?? 'U', 0, 1) }}
                    </div>
                </div>
                <div class="text-right">
                    <h2 class="text-3xl font-extrabold text-gray-900 mb-1">{{ $user->name }}</h2>
                    <p class="text-red-600 font-bold uppercase tracking-wide text-sm">{{ $user->role ?? 'Usuario' }}</p>
                </div>
            </div>
            ...
            <div class="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="p-5 bg-gray-50 rounded-2xl border border-gray-100">
                    ...
                    <p class="text-lg font-semibold text-gray-800">{{ $user->role ?? 'Usuario' }}</p>
                </div>
                <div class="p-5 bg-gray-50 rounded-2xl border border-gray-100">
                    <p class="text-xs text-gray-400 uppercase font-bold tracking-wider mb-1">Correo Electrónico</p>
                    <p class="text-lg font-semibold text-gray-800">{{ $user->email }}</p>
                </div>
                <div class="p-5 bg-gray-50 rounded-2xl border border-gray-100 md:col-span-2">
